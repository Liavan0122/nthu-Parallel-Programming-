...
void LinearLayer(float *A, float *B, float *C, float *D, int n, int k, int m) {
    #pragma acc parallel loop gang copyin(A[0:n*k], B[0:k*m], C[0:m]) copyout(D[0:n*m])
    for (int i = 0; i < n; i++) {
...
void Sigmoid(float *A, int n, int m) {
    #pragma acc parallel loop collapse(2) copyin(A[0:n*m]) copyout(A[0:n*m])
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            // Sigmoid(x) = 1/(1+exp(x))
...
void Argmax(float *A, int *D, int n, int m) {
    #pragma acc parallel loop gang copyin(A[0:n*m]) copyout(D[0:n])
    for (int i = 0; i < n; i++) {
        float mx = A[i * m];
        int index = 0;
        #pragma acc loop seq
