...
/* CUDA kernel for Sobel operation */
            ...
            for (int v = -yBound; v < yBound + adjustY; ++v) {
                for (int u = -xBound; u < xBound + adjustX; ++u) {
                    if ((x + u) >= 0 && (x + u) < width && (y + v) >= 0 && (y + v) < height) {
                        int idx = channels * (width * (y + v) + (x + u));
                        int R = s[idx + 2];
                        int G = s[idx + 1];
                        int B = s[idx + 0];
...
int main(int argc, char** argv) {
    ...
    sobel_kernel<<<gridSize, blockSize>>>(dev_s, dev_t, height, width, channels);
