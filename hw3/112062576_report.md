...
| 只有 1 個 Block，因為只需處理pivot block (r,r)       | dim3(2, round - 1) 兩組執行，分別處理 同行（Row Blocks） 和 同列（Column Blocks）。 且每組負責處理 `round - 1 `個Blocks ，總共需要 2x(round-1) 個 Blocks。 | Time Taken for Mandelbrot Set Calculation                      |   
